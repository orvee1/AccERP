...
import { Save, FileMinus } from 'lucide-react';
...
const DebitNote = () => {
  ...
  const [debitAccount, setDebitAccount] = useState(''); // Account to be debited (e.g., Purchase Returns, Expense)
  const [creditAccount, setCreditAccount] = useState(''); // Account to be credited (typically Vendor/Party)
  const [amount, setAmount] = useState('');
  const [narration, setNarration] = useState('');
  const [reference, setReference] = useState(''); // e.g. Original Purchase Bill No.
  const [debitNoteNumber, setDebitNoteNumber] = useState(`DN-${String(Date.now()).slice(-6)}`);
  ...
  const handleSubmit = (e) => {
    ...
    const existingDebitNotes = JSON.parse(localStorage.getItem('debitNotes') || '[]');
    localStorage.setItem('debitNotes', JSON.stringify([...existingDebitNotes, debitNoteData]));
    toast({
      title: "Debit Note Saved",
      description: `Debit Note ${debitNoteNumber} for ${parseFloat(amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })} has been saved.`,
    ...
    setNarration('');
    setReference('');
    setDebitNoteNumber(`DN-${String(Date.now()).slice(-6)}`);
  };
      ...
      <Card className="shadow-xl border-border dark:border-dark-border">
        <CardHeader className="bg-muted/50 dark:bg-dark-muted/50 p-6 border-b border-border dark:border-dark-border">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <CardTitle className="text-3xl font-bold text-primary dark:text-dark-primary flex items-center">
              <FileMinus size={32} className="mr-3 text-accent dark:text-dark-accent" /> New Debit Note
            </CardTitle>
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="debitAccount" className="font-semibold">Debit Account <span className="text-destructive">*</span></Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="creditAccount" className="font-semibold">Credit Account (Party/Vendor) <span className="text-destructive">*</span></Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="reference" className="font-semibold">Original Bill/Reference No.</Label>
              <Input id="reference" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="e.g., Original Purchase Bill No." />
            </div>

            <div className="space-y-2">
              <Label htmlFor="narration" className="font-semibold">Reason/Narration</Label>
              <Textarea id="narration" value={narration} onChange={(e) => setNarration(e.target.value)} placeholder="e.g., Goods returned, Price correction" />
            </div>
          </CardContent>
          <CardFooter className="bg-muted/30 dark:bg-dark-muted/30 rounded-b-lg p-6 flex justify-end space-x-3 border-t border-border dark:border-dark-border">
            ...
            <Button type="submit" className="bg-primary text-primary-foreground hover:bg-primary/90 dark:bg-dark-primary dark:text-dark-primary-foreground dark:hover:bg-dark-primary/90">
              <Save size={18} className="mr-2" /> Save Debit Note
            </Button>
          </CardFooter>
        </form>
      </Card>
