...
const PurchaseReturn = () => {
  ...
  const [returnNumber, setReturnNumber] = useState(`PRTN-${String(Date.now()).slice(-6)}`);
 ...
 const calculateLineItemTotal = useCallback((item) => {
  ...
  const handleSaveReturn = (isDraft = false) => {
    ...
    const returnData = {
      vendor, returnNumber, returnDate,
    ...
    console.log("Purchase Return Data: ", JSON.stringify(returnData, null, 2));

    const existingReturns = JSON.parse(localStorage.getItem('purchaseReturns') || '[]');
    localStorage.setItem('purchaseReturns', JSON.stringify([...existingReturns, returnData]));

    // Update product stock (decrease stock for purchase returns)
    ...
    toast({ title: "Purchase Return Saved", description: `Return ${returnNumber} has been ${isDraft ? 'saved as draft' : 'saved'}. Stock updated.`, className: 'bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-200 border-green-300 dark:border-green-600' });
  };
        ...
        <CardHeader className="p-6 border-b border-border dark:border-dark-border">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                 <CardTitle className="text-3xl font-bold text-primary dark:text-dark-primary flex items-center">
                    <RotateCcw size={32} className="mr-3 text-accent dark:text-dark-accent" /> New Purchase Return
                </CardTitle>
        ...
        <CardContent className="p-6 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-3 p-4 bg-muted/70 dark:bg-dark-muted/70 rounded-lg shadow-inner border border-border dark:border-dark-border">
              ...
              <div className="flex justify-between items-center">
                <Label htmlFor="taxAmountReturn" className="font-medium">Tax Adjustment:</Label>
